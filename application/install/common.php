    ...
    if ($conn) {
        ...
        if (!@db_select($db_name, $conn)) {
            ...
            if (!@db_query($sqlstr, $conn)) {
                echo json_encode(['status' => 0, 'info' => '创建数据库失败,请检查权限']);
                exit();
            } else {
                echo json_encode(['status' => 1, 'info' => '创建数据库成功', 'sql_mode' => $sql_status]);
                exit();
            }
        } else {
            echo json_encode(['status' => 1, 'info' => '连接数据库成功', 'sql_mode' => $sql_status]);
            exit();
        }
    } else {
        echo json_encode(['status' => 0, 'info' => '数据库连接失败,请检查用户名和密码']);
    }
}
...
function sqlCallBack($sql, $result, $percent, $is_test = false)
{
    ...
    if (preg_match_all("/(create|drop|insert|replace into)([^`]+`)(\w+)(`.*)/i", $sql, $out)) {
        ...
        if ($op == 'create' && $sql) {
            ...
            $message = "校验表 " . ($out[3][0]) . " ";
        } elseif ($op == 'insert' && $sql) {
            $message = "写入表 " . ($out[3][0]) . " ";
        } elseif ($op == 'replace into' && $sql) {
            $message = "写入表 " . ($out[3][0]) . " ";
        }
        //判断sql执行结果
        if ($result) {
...
function install_sql()
{
    ...
    //链接mysql数据库
    $mysql_link = version_compare(phpversion(), '7.1.0') > -1 ? mysqli_connect($db_address . ':' . $db_port, $db_user, $db_pwd):@mysql_connect($db_address . ':' . $db_port, $db_user, $db_pwd);
    if (!$mysql_link) {
        showProgress(['isError' => true, 'message' => 'mysql链接失败' . mysqli_error($mysql_link)]);
    } else {
        showProgress(['isError' => false, 'message' => '连接数据库成功']);
    }
    //检测SQL安装文件
    $sql_file = APP_PATH . '/install/sql/install.sql';
    if (!file_exists($sql_file)) {
        showProgress(['isError' => true, 'message' => '安装的SQL文件' . basename($sql_file) . '不存在']);
    } else {
        showProgress(['isError' => false, 'message' => '解析SQL文件']);
    }
    //执行SQL,创建数据库操作
    ...
    if (!@db_select($db_name, $mysql_link)) {
        ...
        if (!db_query('CREATE DATABASE `' . $db_name . '` ' . $DATABASESQL, $mysql_link)) {
            showProgress(['isError' => true, 'message' => '用户权限受限，创建' . $db_name . '数据库失败，请手动创建数据表']);
        }
    }
    if (!@db_select($db_name, $mysql_link)) {
        showProgress(['isError' => true, 'message' => $db_name . '数据库不存在' . mysqli_error($mysql_link)]);
    }
    //安装SQL
    ...
    if (!file_exists($sql_data_file)) {
        ...
        showProgress(['isError' => false, 'message' => '正在加载必要数据', 'percent' => 90]);
        $sqls = parseSQL($sql_data_file);
        execSQL($sqls, $mysql_link);
        showProgress(['isError' => false, 'message' => '更新必要数据成功', 'percent' => 99]);
    }
    //写入数据库配置文件
    $configDefFile = <<<EOF
<?php
...
EOF;
    $configFile = CONF_PATH . 'database.php';
    $updateData = [
        '{DB_TYPE}' => $db_type,
        '{DB_PREFIX}' => $db_pre,
        '{DB_HOST}' => $db_address,
    ...
    if (!$is_success) {
        showProgress(['isError' => true, 'message' => '更新数据库配置文件失败', 'percent' => 99]);
    } else {
        showProgress(['isError' => false, 'message' => '更新数据库配置文件成功', 'percent' => 99]);
    }
    //插入管理员数据
    $adminSql = 'insert into `' . $db_pre . 'admin` (`login_name`,`pwd`,`type`) values ("' . $admin_user . '","' . pwdHash($admin_pwd) . '",1)';
    if (!db_query($adminSql, $mysql_link)) {
        showProgress(['isError' => true, 'message' => '创建管理员失败' . $adminSql . mysqli_error($mysql_link), 'percent' => 99]);
    } else {
        showProgress(['isError' => false, 'message' => '创建管理员成功', 'percent' => 99]);
    }
    
    showProgress(['isError' => false, 'message' => '更新缓存成功', 'percent' => 99]);
    //执行完毕
    showProgress(['isError' => false, 'message' => '安装完成', 'percent' => 100, 'admin_user' => $admin_user]);
    exit;
}
